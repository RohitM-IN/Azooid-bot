...
module.exports = async (client, guild,user   ) => {
    ...
    //             .setDescription(`**USER**`,`${messageDelete.author.tag}`)
    //             .addField(`**Content**`,` \`\`\`${messageDelete.content}\`\`\` `) //what can i do to put oldMessage.cleanContent in codeblock
    //             .addField(`**Channel**`,`<#${messageDelete.channel.id}> (${messageDelete.channel.id})`)
    //             .setFooter(``)
    //             .setFooter(`MessageID:${messageDelete.author.lastMessageID} | AuthorID:${messageDelete.author.id}`);
    //             log.send(sEmbed);
