...
exports.run = async (client, message, args) => {
    ...
    if(args[0]>100)
    {
        message.channel.send("please enter amount less than 100").then(msg => msg.delete({ timeout: 5000 }));
    }
    else{
        message.channel.bulkDelete(args[0],true).then(() => {
            message.channel.send(`Cleared ${args[0]} messages.`).then(msg => msg.delete({ timeout: 5000 })).catch(err => message.channel.send('There are no messages younger than two weeks that can be deleted.').then(m => m.delete({ timeout: 5000 })));
        })
    }
