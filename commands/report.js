...
exports.run = async (client, message, args) => {
    ...
    if (rMember.hasPermission("BAN_MEMBERS"))
        return message.channel.send("Can't report that member").then(m => m.delete({
            timeout: 5000
        }));
    ...
    if (!channel)
        return message.channel.send("Couldn't find a `#reports` or `#log` channel").then(m => m.delete({
            timeout: 5000
        }));
